...
### Fixed
- **Home Assistant MQTT bridge stopped publishing after hours and never recovered** — once the broker connection dropped (broker restart, network blip, NAT idle timeout) the bridge could go silent permanently, so Home Assistant entities stopped updating and went stale/unavailable. The root cause was a shared stop flag that, once set by a settings change, stayed set and made the very next reconnect attempt exit the connection loop for good; publish failures were also swallowed at `DEBUG`, hiding the problem. The reconnection logic now uses task cancellation instead of the shared flag (with a cancellable back-off between attempts), publish errors are logged and flag the connection as dropped to trigger a prompt reconnect, and discovery configs are re-published on every reconnect so entities are recreated even after a broker/HA restart. Added a bridge availability topic with an MQTT Last Will (`<topic_prefix>/bridge/state` → `online`/`offline`), wired into every discovery config, so Home Assistant now correctly shows the entities as unavailable while the bridge is offline and live again on reconnect.
...
- **Zone dwell & absence alerts** — each zone can set a per-zone "alert if inside longer than" and "alert if away longer than" threshold (minutes; 0 = off). New `zone_dwell` / `zone_absence` notification events fire once per stay and are recorded in history. Backed by entry/exit timers in `ZoneTracker.due_alerts` and migration `0013` (with a self-healing fallback).
- **Zone presence surfacing + Home Assistant** — `GET /api/zones/presence` exposes which zones each vehicle is currently inside; the vehicle selector shows an "at <zone>" badge. The MQTT/Home Assistant bridge publishes a per-vehicle **Current Zone** sensor (zone name, or "Away") with discovery.
- **Charging-by-zone analytics** — each charging session is tagged with the zone it started in (`zone_name`, via migration `0014` + self-healing); the Charging tab shows the zone per session and a **"Charging by location"** energy/cost summary.
...
- **Updated leapmotor-api to v0.3.2**: fixes the GPS hemisphere sign loss on B10/C10 (#21) and the charge-limit/schedule reset bug (#18)
...
- **History day navigation fetched the wrong day in positive-UTC timezones** (#23) — the day-stepper built the `from_date`/`to_date` query from `Date.toISOString()`, which is UTC, while the visible label used the local date; at UTC+ offsets (e.g. Italy) the request asked for the previous day, so the loaded data lagged the label by one day until a refresh. Date ranges are now serialized from the local calendar date, and range requests carry a sequence guard so a slow response for a previous day can no longer overwrite a newer navigation.
- **Send-destination address is now optional** (#14) — non-ASCII street names (e.g. Polish addresses) could crash the T03 head-unit's navigator, while sending the full address otherwise did nothing. The address field is now optional in the UI (send is enabled with just latitude/longitude), and when it is omitted the backend sends a plain-ASCII `"<lat>, <lon>"` coordinate label as both `address` and `addressname` so the head unit always receives a parseable, special-character-free target.
- **Vehicles in the Western hemisphere (e.g. Portugal, UK) were shown in Italy on the map** (#21) — on B10/C10 vehicles, the cloud's primary GPS signals carry longitude/latitude as absolute values, dropping the West/South sign; LeapConnect rendered this unsigned value as-is. Upgraded `leapmotor-api` to 0.3.2, which now prefers the signed GPS signals and falls back to the absolute-value ones only when the signed pair is absent.
- **Setting a charge limit also reset the scheduled charging start time to 00:00** (#18) — for a charge plan that only sets a start time (no end time/recharge cycles), the cloud omits those fields from its response; `leapmotor-api`'s previous read-modify-write logic treated their absence as "no schedule configured" and overwrote `starttime`/`chargeEnable` with defaults when only the charge limit was meant to change. Upgraded `leapmotor-api` to 0.3.2, which now preserves every field already present in the response and only changes the requested charge limit.
